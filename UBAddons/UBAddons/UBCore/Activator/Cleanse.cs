    ...
    class Cleanse
    {
        internal static void OnBuffGain(Obj_AI_Base sender, Obj_AI_BaseBuffGainEventArgs args)
        {
            ...
            if (Sender.IsMe)
            {
                ...
                {
                    if (Main.Clean["Cleanse"] == null || !Main.Clean.VChecked("Cleanse.Enabled")) return;
                    if (Main.Clean["Cleanse." + args.Buff.Type] == null || !Main.Clean.VChecked("Cleanse." + args.Buff.Type)) return;
                ...
                else
                {
                    ...
                    if (Main.Clean[local] == null || !Main.Clean.VChecked(local + ".Enabled")
                        || !Main.Clean.VChecked(local + ".Enabled." + Sender.ChampionName)) return;
                    if (Main.Clean[local + "." + args.Buff.Type] == null || !Main.Clean.VChecked(local + "." + args.Buff.Type)) return;
                    var Duration = (args.Buff.EndTime - Game.Time) * 1000;
            ...
            else
            {
                ...
                if (Main.Clean[mikael.Id.ToString()] == null || !Main.Clean.VChecked(mikael.Id + ".Enabled") 
                    || !Main.Clean.VChecked(mikael.Id + ".Enabled." + Sender.ChampionName)) return;
                if (Main.Clean[mikael.Id + "." + args.Buff.Type] == null || !Main.Clean.VChecked(mikael.Id + "." + args.Buff.Type)) return;
                var Duration = (args.Buff.EndTime - Game.Time) * 1000;
